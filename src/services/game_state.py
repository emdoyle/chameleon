...
class GameStateService:
    ...
    def add_session_to_game(self, session_id: int, game_id: int):
        self.db_session.query(Session).filter(Session.id == session_id).update({'game_id': game_id})
    ...
    def session_ids_for_game(self, game_id: int) -> Set[int]:
        sessions_in_game = self.db_session.query(Session).join(Game, Game.id == Session.game_id).filter(
            Game.id == game_id
        ).all()
