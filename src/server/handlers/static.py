...
from src.db import DBSession, Game, Session, Round
...
class PublicAssetHandler(StaticFileHandler):
    async def get(self, path: str, include_body: bool = True):
        if path in CARD_FILE_NAMES:
            logger.error(
                "Cannot serve protected file: %s from PublicAssetHandler", path
            )
    ...
    async def get(self, path: str, include_body: bool = True):
        ...
        if path == KEYCARD_FILE_NAME:
            ...
            current_round = (
                ...
                .filter(Game.id == validated_session.game_id)
                .first()
            )
            if (
                current_round.phase
                == "set_up"  # TODO: should _really_ be an ordered Enum
                or current_round.set_up_phase.chameleon_session_id
