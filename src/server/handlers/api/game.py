...
class GameAPIHandler(RequestHandler):
    async def get(self):
        ...
        db_session.query(Session).filter(Session.id == int(session_id)).update(
    ...
    async def post(self):
        ...
        db_session = DBSession()
        self._init_game(
            session_id=int(session_id), game_name=game_name, db_session=db_session
        )
        self.set_status(status_code=200)
        self.write(json_encode({"success": True}))
        db_session.close()

    def _init_game(self, session_id: int, game_name: str, db_session: "DBSession"):
        new_game = Game(name=game_name)
        db_session.add(new_game)
        db_session.commit()
        logger.info("Committed game!")

        new_round = Round(game_id=new_game.id,)
