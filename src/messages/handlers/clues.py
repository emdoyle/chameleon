...
class ClueMessageHandler(BaseMessageHandler):
    ...
    def _update_game_state(self, session: 'Session', clue: str, connected_sessions: Set[int]) -> None:
    ...
    def handle(self, message: Dict, session: 'Session') -> 'OutgoingMessages':
        ...
        connected_sessions = GameStateService(db_session=self.db_session).connected_sessions_in_game(session.game_id)
