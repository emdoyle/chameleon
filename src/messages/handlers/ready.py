...
from src.db import SetUpPhase, Round
...
class ReadyMessageHandler(BaseMessageHandler):
    ...
    def _handle_full_ready(self, game_id: int) -> None:
        ...
        session_ids = {
            session.id for session in self._get_sessions_in_game(game_id=game_id)
        }
        set_up_phase = (
            self.db_session.query(SetUpPhase)
            .join(Round, SetUpPhase.round_id == Round.id)
        ...
        game_round = (
            ...
            .filter(Round.id == set_up_phase.round_id)
            .first()
        )
