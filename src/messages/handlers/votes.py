...
class VoteMessageHandler(BaseMessageHandler):
    def _validate_vote(self, vote: str, game_id: int) -> bool:
        ...
        return vote in {player.username for player in connected_players}
    ...
    def _get_final_vote(self, votes: Dict[str, str], game_id: int) -> Optional[str]:
        ...
        if (
            {int(key) for key in votes.keys()}
            == self._get_connected_sessions(game_id=game_id)
            and len(counted_votes) <= 2
            and (
                len(counted_votes) == 1
                or any(count <= 1 for count in counted_votes.values())
            )
        ):
    ...
    def _update_game_state(self, session: "Session", vote: Optional[str]) -> None:
        ...
        if vote is None:
            vote_phase.votes = {
                key: value
                for key, value in vote_phase.votes.items()
        ...
        if vote_phase.final_vote is not None:
            ...
            chameleon_username = self._get_username_for_session_id(
                self._get_chameleon_session_id(session.game_id)
            )
