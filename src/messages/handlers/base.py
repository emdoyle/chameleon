...
from src.db import Session, Game, Round, SetUpPhase, CluePhase, RevealPhase, User
...
class BaseMessageHandler(AbstractMessageHandler):
    def __init__(self, db_session: "DBSession", game_id: int):
        self.db_session = db_session
        self.game_id = game_id
        self.message_builder = MessageBuilder.factory(
            db_session=db_session, game_id=game_id
        )

    @classmethod
    def factory(cls, db_session: "DBSession", game_id: int):
        return cls(db_session=db_session, game_id=game_id)
    ...
    def _get_set_up_phase(self, game_id: int) -> Optional["SetUpPhase"]:
        return (
            self.db_session.query(SetUpPhase)
            .join(Round, Round.id == SetUpPhase.round_id)
    ...
    def _get_clue_phase(self, game_id: int) -> Optional["CluePhase"]:
        return (
            self.db_session.query(CluePhase)
            .join(Round, Round.id == CluePhase.round_id)
            .join(Game, Game.id == Round.game_id)
            .filter(Round.completed == false())
            .filter(Game.id == game_id)
            .first()
    ...
    def _get_reveal_phase(self, game_id: int) -> Optional["RevealPhase"]:
        return (
            self.db_session.query(RevealPhase)
            .join(Round, Round.id == RevealPhase.round_id)
            .join(Game, Game.id == Round.game_id)
            .filter(Round.completed == false())
            .filter(Game.id == game_id)
            .first()
    ...
    def _get_connected_players(self, game_id: int) -> Iterable["User"]:
        ...
        return (
            self.db_session.query(User)
            .join(Session, Session.user_id == User.id)
            .filter(Session.id.in_(connected_session_ids))
            .all()
        )
    ...
    def _get_chameleon_session_id(self, game_id: int) -> Optional[int]:
        ...
        if set_up_phase is None:
            logger.debug(
                "Tried to get chameleon session id but none found with game id: %s",
                game_id,
            )
            return None
        return set_up_phase.chameleon_session_id

    def _get_clue_turn_session_id(
        self, game_id: int, connected_sessions: Set[int]
    ) -> Optional[int]:
        ...
        if set_up_phase is None:
            logger.debug(
                "Tried to get clue turn session id but none found with game id: %s",
                game_id,
            )
            return None
        ...
        return next(
            (
                ...
                if str(session_id) not in clue_phase.clues
                and session_id in connected_sessions
            ),
            None,
        )
    ...
    def _default_messages(
        ...
        game_state_service = GameStateService(db_session=self.db_session)
        connected_sessions = game_state_service.connected_sessions_in_game(
            game_id=game_id
        )
        ...
        for session_in_game in sessions_in_game:
            ...
            if (
                chameleon_session_id is not None
                and session_in_game.id == chameleon_session_id
            ):
                logger.debug(
                    "Showing session %s that they are the chameleon!",
                    session_in_game.id,
                )
                message_to_send = full_game_state_message.add_chameleon()
            if (
                clue_turn_session_id is not None
                and session_in_game.id == clue_turn_session_id
            ):
                logger.debug(
                    "Showing session %s that it is their turn to give a clue!",
                    session_in_game.id,
                )
