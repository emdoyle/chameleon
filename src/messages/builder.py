...
from src.db import DBSession, User, Round, Session
from src.categories import decode
from src.settings import LOGGER_NAME
from .data import OutgoingMessage

if TYPE_CHECKING:
    from src.db import RevealPhase, VotePhase, CluePhase, SetUpPhase
    ...
    def create_full_game_state_message(
        ...
        if (
            ...
            players_in_game = players_in_game.order_by(
                func.array_position(
                    first_uncompleted_round.set_up_phase.session_ordering, Session.id
                )
            )
        ...
        return OutgoingMessage(data={**round_dict, **players_dict},)
